...
- (dispatch_queue_t)executeModify:(blk_t)blockTask;
- (instancetype)init;
...
- (instancetype)init {
    if (self = [super init]) {
        dataSerialQueue = dispatch_queue_create(SERIAL_QUEUE_NAME, NULL);
    }
    return self;
}
...
+ (id<PCRUD>)sharedInstance
{
    ...
    dispatch_once(&onceToken, ^{
        sharedInstance = [[LABData alloc] init];
...
- (NSArray*) select:(Class)clazz filter:(NSDictionary*)where sortBy:(NSArray*)sort inAsc:(BOOL)asc {
    ...
    //dispatch_time_t timeoutTime = dispatch_time(DISPATCH_TIME_NOW, 3000000000);
...
- (NSArray*) select:(NSString*)sql selectionArgs:(NSArray*)selectionArgs {
    ...
    dispatch_time_t timeoutTime = dispatch_time(DISPATCH_TIME_NOW, 3000000000);
    dispatch_semaphore_wait(sem, timeoutTime);
    return list;
}
...
- (long long) delete:(id)object {
    ...
    [self executeModify:^ {
        ...
        row = [self delete:klass where:where];
    }];
    return row;
}
...
- (NSArray*)getModels:(Class)model fromStatement:(sqlite3_stmt*)statement {
    ...
    while (sqlite3_step(statement) == SQLITE_ROW) {
        ...
        for (char i = 0; i < columnCount; i++) {
            ...
            if ([columnName isEqualToString:@"description"]) {
            ...
            } else if (sqlite3_column_type(statement, i) == SQLITE_TEXT && [dataType isEqualToString:@"NSString"]) {
                char *characters = (char*)sqlite3_column_text(statement, i);
                [row setValue:[NSString stringWithCString:characters encoding:NSUTF8StringEncoding] forKey:columnName];
            }
                ...
                [row setValue:[NSNumber numberWithDouble:decimalNumber] forKey:columnName];
            }
        }
        [list addObject:row];
    }
...
/**
 * Create a serial thread to execute a block.
 */
- (dispatch_queue_t)executeModify:(blk_t)blockTask {
    if (dataSerialQueue == NULL) {
        dataSerialQueue = dispatch_queue_create(SERIAL_QUEUE_NAME, NULL);
    }
    
    dispatch_async(dataSerialQueue, blockTask);
